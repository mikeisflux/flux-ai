...
import { accumulateUsage, updateUsage } from 'src/services/api/flux.js'
  ...
  async *submitMessage(
    ...
    if (persistSession && messagesFromUserInput.length > 0) {
      ...
      if (isBareMode()) {
        ...
        if (
          isEnvTruthy(process.env.FLUX_AI_EAGER_FLUSH) ||
          isEnvTruthy(process.env.FLUX_AI_IS_COWORK)
        ) {
    ...
    // ref-tracked plugins. CCR populates the cache via FLUX_AI_SYNC_PLUGIN_INSTALL
    // (headlessPluginInstall) or FLUX_AI_PLUGIN_SEED_DIR before this runs;
