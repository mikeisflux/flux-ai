 ...
 * with the claude.ai OAuth token). isFluxAISubscriber() excludes
 ...
 * (setup-token, FLUX_AI_OAUTH_TOKEN env var, or pre-scope-expansion
...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    ...
    if (!hasProfileScope()) {
      return 'Remote Control requires a full-scope login token. Long-lived tokens (from `claude setup-token` or FLUX_AI_OAUTH_TOKEN) are limited to inference-only for security reasons. Run `claude auth login` to use Remote Control.'
    }
...
// program, before enableConfigs() runs. isFluxAISubscriber() → getGlobalConfig()
...
function isFluxAISubscriber(): boolean {
  try {
    return authModule.isFluxAISubscriber()
  } catch {
    return false
  }
}
...
export function isCcrMirrorEnabled(): boolean {
  return feature('CCR_MIRROR')
    ? isEnvTruthy(process.env.FLUX_AI_CCR_MIRROR) ||
