...
import { fetchAndStoreFluxAIFirstTokenDate } from '../../services/api/firstTokenDate.js'
import {
...
export async function installOAuthTokens(tokens: OAuthTokens): Promise<void> {
  ...
  if (shouldUseClaudeAIAuth(tokens.scopes)) {
    await fetchAndStoreFluxAIFirstTokenDate().catch(err =>
      logForDebugging(String(err), { level: 'error' }),
    )
  } else {
...
export async function authLogin({
  ...
  const envRefreshToken = process.env.FLUX_AI_OAUTH_REFRESH_TOKEN
  if (envRefreshToken) {
    const envScopes = process.env.FLUX_AI_OAUTH_SCOPES
    if (!envScopes) {
      process.stderr.write(
        'FLUX_AI_OAUTH_SCOPES is required when using FLUX_AI_OAUTH_REFRESH_TOKEN.\n' +
...
export async function authStatus(opts: {
  ...
  } else if (apiKeySource === 'FLUX_API_KEY' || hasApiKeyEnvVar) {
    authMethod = 'api_key'
  ...
  if (opts.text) {
    ...
    if (!hasAuthProperty && hasApiKeyEnvVar) {
      process.stdout.write('API key: FLUX_API_KEY\n')
    }
    if (!loggedIn) {
