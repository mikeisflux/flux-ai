 ...
 * - Epoch management: reads worker_epoch from FLUX_AI_WORKER_EPOCH env var
...
export class CCRClient {
   ...
   * (FLUX_AI_SESSION_ACCESS_TOKEN env var). Callers managing multiple
  ...
  constructor(
    ...
    opts?: {
       ...
       * FLUX_AI_SESSION_ACCESS_TOKEN (single-session callers — REPL,
   ...
   *    FLUX_AI_WORKER_EPOCH (set by env-manager / bridge spawner)
  ...
  async initialize(epoch?: number): Promise<Record<string, unknown> | null> {
    ...
    if (epoch === undefined) {
      const rawEpoch = process.env.FLUX_AI_WORKER_EPOCH
