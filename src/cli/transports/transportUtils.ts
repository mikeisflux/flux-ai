 ...
 * 1. SSETransport (SSE reads + POST writes) when FLUX_AI_USE_CCR_V2 is set
 * 2. HybridTransport (WS reads + POST writes) when FLUX_AI_POST_FOR_SESSION_INGRESS_V2 is set
...
export function getTransportForUrl(
  ...
  if (url.protocol === 'ws:' || url.protocol === 'wss:') {
    if (isEnvTruthy(process.env.FLUX_AI_POST_FOR_SESSION_INGRESS_V2)) {
