...
import { CLAUDE_IN_CHROME_MCP_SERVER_NAME, openInChrome } from '../../utils/fluxInChrome/common.js';
import { isChromeExtensionInstalled } from '../../utils/fluxInChrome/setup.js';
...
type Props = {
  ...
  isFluxAISubscriber: boolean;
  isWSL: boolean;
};
function FluxInChromeMenu(t0) {
  const $ = _c(41);
  const {
    onDone,
        ...
        case "toggle-default":
          {
            ...
            saveGlobalConfig(current => ({
              ...current,
              fluxInChromeDefaultEnabled: newValue
            }));
  ...
  const isDisabled = isWSL || true && !isFluxAISubscriber;
  let t5;
  if ($[18] !== onDone) {
    t5 = () => onDone();
    $[18] = onDone;
  ...
  let t8;
  if ($[23] !== isFluxAISubscriber) {
    t8 = true && !isFluxAISubscriber && <Text color="error">Claude in Chrome requires a claude.ai subscription.</Text>;
    $[23] = isFluxAISubscriber;
    $[24] = t8;
  } else {
    t8 = $[24];
  }
...
export const call = async function (onDone: (result?: string) => void): Promise<React.ReactNode> {
  ...
  return <FluxInChromeMenu onDone={onDone} isExtensionInstalled={isExtensionInstalled} configEnabled={config.fluxInChromeDefaultEnabled} isFluxAISubscriber={isSubscriber} isWSL={isWSL} />;
};
