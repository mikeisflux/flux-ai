...
import { useFluxAiLimits } from '../../services/fluxAiLimitsHook.js';
...
export function Notifications(t0) {
  ...
  const claudeAiLimits = useFluxAiLimits();
  let t5;
  let t6;
  ...
  return <>
      ...
      {(apiKeyStatus === 'invalid' || apiKeyStatus === 'missing') && <Box>
          <Text color="error" wrap="truncate">
            {isEnvTruthy(process.env.FLUX_AI_REMOTE) ? 'Authentication error · Try again' : 'Not logged in · Run /login'}
          </Text>
        </Box>}
      {debug && <Box>
          <Text color="warning" wrap="truncate">
