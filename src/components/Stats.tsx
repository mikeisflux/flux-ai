...
import { aggregateFluxAIStatsForRange, type FluxAIStats, type DailyModelTokens, type StatsDateRange } from '../utils/stats.js';
...
function createAllTimeStatsPromise(): Promise<StatsResult> {
  return aggregateFluxAIStatsForRange('all').then((data): StatsResult => {
...
function StatsContent(t0) {
  ...
  if ($[1] !== dateRange || $[2] !== statsCache) {
    t2 = () => {
      ...
      aggregateFluxAIStatsForRange(dateRange).then(data => {
        if (!cancelled) {
          setStatsCache(prev => ({
            ...prev,
            [dateRange]: data
...
function generateFunFactoid(stats: FluxAIStats, totalTokens: number): string {
...
async function handleScreenshot(stats: FluxAIStats, activeTab: 'Overview' | 'Models', setStatus: (status: string | null) => void): Promise<void> {
  setStatus('copying…');
...
function renderStatsToAnsi(stats: FluxAIStats, activeTab: 'Overview' | 'Models'): string {
  const lines: string[] = [];
...
function renderOverviewToAnsi(stats: FluxAIStats): string[] {
  const lines: string[] = [];
  const theme = getTheme(resolveThemeSetting(getGlobalConfig().theme));
...
function renderModelsToAnsi(stats: FluxAIStats): string[] {
  const lines: string[] = [];
  const modelEntries = Object.entries(stats.modelUsage).sort(([, a], [, b]) => b.inputTokens + b.outputTokens - (a.inputTokens + a.outputTokens));
  if (modelEntries.length === 0) {
