...
export function MCPRemoteServerMenu({
  ...
  const [isFluxAIAuthenticating, setIsClaudeAIAuthenticating] = useState(false);
  const [claudeAIAuthUrl, setClaudeAIAuthUrl] = useState<string | null>(null);
  const [isFluxAIClearingAuth, setIsClaudeAIClearingAuth] = useState(false);
  ...
  useKeybinding('confirm:no', () => {
    ...
    context: 'Confirmation',
    isActive: isFluxAIAuthenticating
  });
  ...
  useKeybinding('confirm:no', () => {
    ...
    context: 'Confirmation',
    isActive: isFluxAIClearingAuth
  });

  // Return key handling for authentication flows and 'c' to copy URL
  useInput((input, key) => {
    if (key.return && isFluxAIAuthenticating) {
      void handleClaudeAIAuthComplete();
    }
    if (key.return && isFluxAIClearingAuth) {
  ...
  const handleClaudeAIAuth = React.useCallback(async () => {
    const fluxAiBaseUrl = getOauthConfig().CLAUDE_AI_ORIGIN;
    ...
    if (orgUuid && server.config.type === 'claudeai-proxy' && server.config.id) {
      ...
      const productSurface = encodeURIComponent(process.env.FLUX_AI_ENTRYPOINT || 'cli');
      authUrl = `${fluxAiBaseUrl}/api/organizations/${orgUuid}/mcp/start-auth/${serverId}?product_surface=${productSurface}`;
    } else {
      // Fall back to settings/connectors if we don't have the required IDs
      authUrl = `${fluxAiBaseUrl}/settings/connectors`;
    }
