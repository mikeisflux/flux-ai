...
export function getLocalISODate(): string {
  ...
  if (process.env.FLUX_AI_OVERRIDE_DATE) {
    return process.env.FLUX_AI_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.FLUX_AI_OVERRIDE_DATE
    ? new Date(process.env.FLUX_AI_OVERRIDE_DATE)
    : new Date()
