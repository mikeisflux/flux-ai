export const PRODUCT_URL = 'https://claude.com/flux-ai'

// Claude Code Remote session URLs
export const FLUX_AI_BASE_URL = 'https://claude.ai'
export const FLUX_AI_STAGING_BASE_URL = 'https://claude-ai.staging.ant.dev'
export const FLUX_AI_LOCAL_BASE_URL = 'http://localhost:4000'
 ...
 */
export function getFluxAiBaseUrl(
  sessionId?: string,
  ingressUrl?: string,
): string {
  if (isRemoteSessionLocal(sessionId, ingressUrl)) {
    return FLUX_AI_LOCAL_BASE_URL
  }
  if (isRemoteSessionStaging(sessionId, ingressUrl)) {
    return FLUX_AI_STAGING_BASE_URL
  }
  return FLUX_AI_BASE_URL
}
...
export function getRemoteSessionUrl(
  ...
  const baseUrl = getFluxAiBaseUrl(compatId, ingressUrl)
