...
if (process.env.FLUX_AI_REMOTE === 'true') {
...
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
if (feature('ABLATION_BASELINE') && process.env.FLUX_AI_ABLATION_BASELINE) {
  for (const k of ['FLUX_AI_SIMPLE', 'FLUX_AI_DISABLE_THINKING', 'DISABLE_INTERLEAVED_THINKING', 'DISABLE_COMPACT', 'DISABLE_AUTO_COMPACT', 'FLUX_AI_DISABLE_AUTO_MEMORY', 'FLUX_AI_DISABLE_BACKGROUND_TASKS']) {
    // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
    process.env[k] ??= '1';
  }
}
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.VERSION} (Flux AI)`);
    return;
  }
  ...
  if (process.argv[2] === '--flux-in-chrome-mcp') {
    profileCheckpoint('cli_flux_in_chrome_mcp_path');
    const {
      runFluxInChromeMcpServer
    } = await import('../utils/fluxInChrome/mcpServer.js');
    await runFluxInChromeMcpServer();
    return;
  } else if (process.argv[2] === '--chrome-native-host') {
    profileCheckpoint('cli_chrome_native_host_path');
    const {
      runChromeNativeHost
    } = await import('../utils/fluxInChrome/chromeNativeHost.js');
