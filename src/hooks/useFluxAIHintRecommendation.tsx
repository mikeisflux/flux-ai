...
/**
 * Surfaces plugin-install prompts driven by `<flux-ai-hint />` tags
 * that CLIs/SDKs emit to stderr. See docs/flux-ai-hints.md.
 *
...
import { clearPendingHint, getPendingHintSnapshot, markShownThisSession, subscribeToPendingHint } from '../utils/fluxAIHints.js';
  ...
  if ($[0] !== pendingHint || $[1] !== tryResolve) {
    t0 = () => {
      ...
      tryResolve(async () => {
        ...
        if (resolved) {
          logForDebugging(`[useFluxAIHintRecommendation] surfacing ${resolved.pluginId} from ${resolved.sourceCommand}`);
