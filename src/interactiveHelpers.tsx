...
import { getExternalClaudeMdIncludes, getMemoryFiles, shouldShowClaudeMdExternalIncludesWarning } from './utils/fluxmd.js';
...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], fluxInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  if (!isEnvTruthy(process.env.CLAUBBIT)) {
    ...
    if (await shouldShowClaudeMdExternalIncludesWarning()) {
      ...
      const {
        FluxMdExternalIncludesDialog
      } = await import('./components/FluxMdExternalIncludesDialog.js');
      await showSetupDialog(root, done => <FluxMdExternalIncludesDialog onDone={done} isStandaloneDialog externalIncludes={externalIncludes} />);
    }
  }
  ...
  if (process.env.FLUX_API_KEY && !isRunningOnHomespace()) {
    const customApiKeyTruncated = normalizeApiKeyForConfig(process.env.FLUX_API_KEY);
  ...
  if (fluxInChrome && !getGlobalConfig().hasCompletedFluxInChromeOnboarding) {
    const {
      FluxInChromeOnboarding
    } = await import('./components/FluxInChromeOnboarding.js');
    await showSetupDialog(root, done => <FluxInChromeOnboarding onDone={done} />);
  }
...
export function getRenderContext(exitOnCtrlC: boolean): {
  ...
  const frameTimingLogPath = process.env.FLUX_AI_FRAME_TIMING_LOG;
  return {
