 ...
 *   1. FLUX_AI_DISABLE_AUTO_MEMORY env var (1/true → OFF, 0/false → ON)
 *   2. FLUX_AI_SIMPLE (--bare) → OFF
 *   3. CCR without persistent storage → OFF (no FLUX_AI_REMOTE_MEMORY_DIR)
 *   4. autoMemoryEnabled in settings.json (supports project-level opt-out)
 *   5. Default: enabled
 */
export function isAutoMemoryEnabled(): boolean {
  const envVal = process.env.FLUX_AI_DISABLE_AUTO_MEMORY
  ...
  if (isEnvTruthy(process.env.FLUX_AI_SIMPLE)) {
    return false
  }
  if (
    isEnvTruthy(process.env.FLUX_AI_REMOTE) &&
    !process.env.FLUX_AI_REMOTE_MEMORY_DIR
  ) {
    return false
  }
 ...
 * Resolution order:
 *   1. FLUX_AI_REMOTE_MEMORY_DIR env var (explicit override, set in CCR)
 *   2. ~/.claude (default config home)
 */
export function getMemoryBaseDir(): string {
  if (process.env.FLUX_AI_REMOTE_MEMORY_DIR) {
    return process.env.FLUX_AI_REMOTE_MEMORY_DIR
  }
 ...
 * same repo share one auto-memory directory (anthropics/flux-ai#24382).
 */
