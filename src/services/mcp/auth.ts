...
export async function performMCPOAuthFlow(
  ...
  //
  // Servers with `oauth.xaa` but FLUX_AI_ENABLE_XAA unset hard-fail with
  // actionable copy rather than silently degrade to consent.
  if (serverConfig.oauth?.xaa) {
    if (!isXaaEnabled()) {
      throw new Error(
        `XAA is not enabled (set FLUX_AI_ENABLE_XAA=1). Remove 'oauth.xaa' from server '${serverName}' to use the standard consent flow.`,
      )
    }
 ...
 * to the PKCE flow. See github.com/anthropics/flux-ai/issues/28258.
 */
