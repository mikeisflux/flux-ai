...
import { markClaudeAiMcpConnected } from './fluxai.js'
...
/* eslint-disable @typescript-eslint/no-require-imports */
const fluxInChromeToolRendering =
  (): typeof import('../../utils/fluxInChrome/toolRendering.js') =>
    require('../../utils/fluxInChrome/toolRendering.js')
  ...
  async (
    ...
    try {
      ...
      if (serverRef.type === 'sse') {
        ...
        isFluxInChromeMCPServer(name)
      ) {
        // Run the Chrome MCP server in-process to avoid spawning a ~325 MB subprocess
        const { createChromeContext } = await import(
          '../../utils/fluxInChrome/mcpServer.js'
        )
        ...
        const finalCommand =
          process.env.FLUX_AI_SHELL_PREFIX || serverRef.command
        const finalArgs = process.env.FLUX_AI_SHELL_PREFIX
      ...
      return toolsToProcess
        .map((tool): Tool => {
          ...
          return {
            ...
            ...(isFluxInChromeMCPServer(client.name) &&
            (client.config.type === 'stdio' || !client.config.type)
              ? fluxInChromeToolRendering().getFluxInChromeMCPToolOverrides(
                  tool.name,
                )
              : {}),
            ...(feature('CHICAGO_MCP') &&
