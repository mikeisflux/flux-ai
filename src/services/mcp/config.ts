...
import { isFluxInChromeMCPServer } from '../../utils/fluxInChrome/common.js'
import {
...
import { fetchClaudeAIMcpConfigsIfEligible } from './fluxai.js'
 ...
 * in getFluxAIMcpConfigs(): --mcp-config (main.tsx) and the mcp_set_servers
...
export async function addMcpConfig(
  ...
  // Block reserved server name "flux-in-chrome"
  if (isFluxInChromeMCPServer(name)) {
...
export function getMcpConfigByName(name: string): ScopedMcpServerConfig | null {
  ...
  // by name. User/project/local servers are blocked — same as getFluxAIMcpConfigs().
 ...
 */
export async function getFluxAIMcpConfigs(
 ...
 * This may be slow due to network calls - use getFluxAIMcpConfigs() for fast startup.
...
export async function getAllMcpConfigs(): Promise<{
  ...
  if (doesEnterpriseMcpConfigExist()) {
    return getFluxAIMcpConfigs()
  }

  // Kick off the claude.ai fetch before getFluxAIMcpConfigs so it overlaps
  ...
  const { servers: fluxAIServers, errors } = await getFluxAIMcpConfigs(
    {},
    claudeaiPromise,
  )
  ...
  const servers = Object.assign({}, dedupedClaudeAi, fluxAIServers)
