...
export async function refreshOAuthToken(
  ...
  try {
    ...
    // FLUX_AI_OAUTH_REFRESH_TOKEN re-login path: installOAuthTokens runs
...
export async function populateOAuthAccountInfoIfNeeded(): Promise<boolean> {
  ...
  const envAccountUuid = process.env.FLUX_AI_ACCOUNT_UUID
  const envUserEmail = process.env.FLUX_AI_USER_EMAIL
  const envOrganizationUuid = process.env.FLUX_AI_ORGANIZATION_UUID
