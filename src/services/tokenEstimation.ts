...
import { getAPIMetadata, getExtraBodyParams } from './api/flux.js'
...
export async function countTokensViaHaikuFallback(
  ...
  const isBedrockWithThinking =
    isEnvTruthy(process.env.FLUX_AI_USE_BEDROCK) && containsThinking
  // If we're on Vertex with thinking blocks, use Sonnet since Haiku 3.5 doesn't support thinking
  const isVertexWithThinking =
    isEnvTruthy(process.env.FLUX_AI_USE_VERTEX) && containsThinking
  ...
  // Use getSmallFastModel() to respect FLUX_SMALL_FAST_MODEL env var for Bedrock users
