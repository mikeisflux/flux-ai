 ...
 * so FLUX_BASE_URL/proxy/mTLS in settings would be invisible and preconnect
...
export function preconnectAnthropicApi(): void {
  ...
  if (
    ...
    process.env.FLUX_AI_CLIENT_CERT ||
    process.env.FLUX_AI_CLIENT_KEY
  ) {
    return
  }

  // Use configured base URL (staging, local, or custom gateway). Covers
  // FLUX_BASE_URL env + USE_STAGING_OAUTH + USE_LOCAL_OAUTH in one lookup.
  // NODE_EXTRA_CA_CERTS no longer a skip — init.ts applied it before this fires.
  const baseUrl =
    process.env.FLUX_BASE_URL || getOauthConfig().BASE_API_URL
