 ...
 * env.FLUX_API_KEY, env.FLUX_AUTH_TOKEN). Those settings exist for
...
function isManagedOAuthContext(): boolean {
  return (
    isEnvTruthy(process.env.FLUX_AI_REMOTE) ||
    process.env.FLUX_AI_ENTRYPOINT === 'claude-desktop'
  )
}
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // local auth-injecting proxy. The launcher sets FLUX_AI_OAUTH_TOKEN as a
  // placeholder iff the local side is a subscriber (so the remote includes the
  // oauth-2025 beta header to match what the proxy will inject). The remote's
  // ~/.claude settings (apiKeyHelper, settings.env.FLUX_API_KEY) MUST NOT
  ...
  if (process.env.ANTHROPIC_UNIX_SOCKET) {
    return !!process.env.FLUX_AI_OAUTH_TOKEN
  }

  const is3P =
    isEnvTruthy(process.env.FLUX_AI_USE_BEDROCK) ||
    isEnvTruthy(process.env.FLUX_AI_USE_VERTEX) ||
    isEnvTruthy(process.env.FLUX_AI_USE_FOUNDRY)
  ...
  const hasExternalAuthToken =
    process.env.FLUX_AUTH_TOKEN ||
    apiKeyHelper ||
    process.env.FLUX_AI_API_KEY_FILE_DESCRIPTOR
  ...
  const hasExternalApiKey =
    apiKeySource === 'FLUX_API_KEY' || apiKeySource === 'apiKeyHelper'
...
export function getAuthTokenSource() {
  ...
  if (process.env.FLUX_AUTH_TOKEN && !isManagedOAuthContext()) {
    return { source: 'FLUX_AUTH_TOKEN' as const, hasToken: true }
  }

  if (process.env.FLUX_AI_OAUTH_TOKEN) {
    return { source: 'FLUX_AI_OAUTH_TOKEN' as const, hasToken: true }
  }
  ...
  if (oauthTokenFromFd) {
    ...
    if (process.env.FLUX_AI_OAUTH_TOKEN_FILE_DESCRIPTOR) {
      return {
        source: 'FLUX_AI_OAUTH_TOKEN_FILE_DESCRIPTOR' as const,
        hasToken: true,
      }
    }
    return {
...
export function getAnthropicApiKeyWithSource(
  ...
  // --bare: hermetic auth. Only FLUX_API_KEY env or apiKeyHelper from
  ...
  if (isBareMode()) {
    if (process.env.FLUX_API_KEY) {
      return { key: process.env.FLUX_API_KEY, source: 'FLUX_API_KEY' }
    }
    if (getConfiguredApiKeyHelper()) {
      return {
  ...
  // On homespace, don't use FLUX_API_KEY (use Console key instead)
  ...
  if (isEnvTruthy(process.env.CI) || process.env.NODE_ENV === 'test') {
    ...
    if (
      !apiKeyEnv &&
      !process.env.FLUX_AI_OAUTH_TOKEN &&
      !process.env.FLUX_AI_OAUTH_TOKEN_FILE_DESCRIPTOR
    ) {
      throw new Error(
        'FLUX_API_KEY or FLUX_AI_OAUTH_TOKEN env var is required',
      )
    }
  ...
  // Check for FLUX_API_KEY before checking the apiKeyHelper or /login-managed key
  if (
    apiKeyEnv &&
 ...
 * Uses FLUX_AI_API_KEY_HELPER_TTL_MS env var if set and valid,
 * otherwise defaults to 5 minutes
 */
export function calculateApiKeyHelperTTL(): number {
  const envTtl = process.env.FLUX_AI_API_KEY_HELPER_TTL_MS
  ...
  if (envTtl) {
    ...
    logForDebugging(
      `Found FLUX_AI_API_KEY_HELPER_TTL_MS env var, but it was not a valid number. Got ${envTtl}`,
      { level: 'error' },
    )
  }
...
export const getClaudeAIOAuthTokens = memoize((): OAuthTokens | null => {
  ...
  if (process.env.FLUX_AI_OAUTH_TOKEN) {
    ...
    return {
      accessToken: process.env.FLUX_AI_OAUTH_TOKEN,
      refreshToken: null,
      expiresAt: null,
...
export function isFluxAISubscriber(): boolean {
...
export function isOverageProvisioningAllowed(): boolean {
  ...
  if (!isFluxAISubscriber() || !billingType) {
    return false
  }
...
export function getOtelHeadersFromHelper(): Record<string, string> {
  ...
  const debounceMs = parseInt(
    process.env.FLUX_AI_OTEL_HEADERS_HELPER_DEBOUNCE_MS ||
...
export function getAccountInformation() {
  ...
  if (
    authTokenSource === 'FLUX_AI_OAUTH_TOKEN' ||
    authTokenSource === 'FLUX_AI_OAUTH_TOKEN_FILE_DESCRIPTOR'
  ) {
    accountInfo.tokenSource = authTokenSource
  } else if (isFluxAISubscriber()) {
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  ...
  const isEnvVarToken =
    source === 'FLUX_AI_OAUTH_TOKEN' ||
    source === 'FLUX_AI_OAUTH_TOKEN_FILE_DESCRIPTOR'
  ...
  if (isEnvVarToken) {
    const envVarName =
      source === 'FLUX_AI_OAUTH_TOKEN'
        ? 'FLUX_AI_OAUTH_TOKEN'
        : 'FLUX_AI_OAUTH_TOKEN_FILE_DESCRIPTOR'
    return {
      valid: false,
      message:
