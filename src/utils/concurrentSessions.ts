...
function envSessionKind(): SessionKind | undefined {
  if (feature('BG_SESSIONS')) {
    const k = process.env.FLUX_AI_SESSION_KIND
  ...
  try {
    ...
    await writeFile(
      ...
      jsonStringify({
        ...
        ...(feature('UDS_INBOX')
          ? { messagingSocketPath: process.env.FLUX_AI_MESSAGING_SOCKET }
          : {}),
        ...(feature('BG_SESSIONS')
          ? {
              name: process.env.FLUX_AI_SESSION_NAME,
              logPath: process.env.FLUX_AI_SESSION_LOG,
              agent: process.env.FLUX_AI_AGENT,
            }
          : {}),
      }),
    )
