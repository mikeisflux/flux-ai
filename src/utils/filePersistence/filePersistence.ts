 ...
 * - Checks environment kind (FLUX_AI_ENVIRONMENT_KIND)
 * - Retrieves session access token
 * - Requires FLUX_AI_REMOTE_SESSION_ID for session ID
 *
...
export async function runFilePersistence(
  ...
  const sessionId = process.env.FLUX_AI_REMOTE_SESSION_ID
  if (!sessionId) {
    logError(
      new Error(
        'File persistence enabled but FLUX_AI_REMOTE_SESSION_ID is not set',
      ),
    )
    return null
  }
