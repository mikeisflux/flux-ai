 ...
 * `<flux-ai-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/flux-ai-hints.md for the vendor-facing spec.
 */

import { logForDebugging } from './debug.js'
import { createSignal } from './signal.js'

export type FluxAIHintType = 'plugin'

export type FluxAIHint = {
 ...
 */
const HINT_TAG_RE = /^[ \t]*<flux-ai-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
...
export function extractFluxAIHints(
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    if (!SUPPORTED_VERSIONS.has(v)) {
      logForDebugging(
        `[fluxAIHints] dropped hint with unsupported v=${attrs.v}`,
      )
      return ''
    }
    if (!type || !SUPPORTED_TYPES.has(type)) {
      logForDebugging(
        `[fluxAIHints] dropped hint with unsupported type=${type}`,
      )
      return ''
    }
    if (!value) {
      logForDebugging('[fluxAIHints] dropped hint with empty value')
      return ''
    }

    hints.push({ v, type: type as FluxAIHintType, value, sourceCommand })
    return ''
  })
...
let pendingHint: FluxAIHint | null = null
...
export function setPendingHint(hint: FluxAIHint): void {
...
export function getPendingHintSnapshot(): FluxAIHint | null {
  return pendingHint
}
...
/** Test-only reset. */
export function _resetFluxAIHintStore(): void {
  pendingHint = null
