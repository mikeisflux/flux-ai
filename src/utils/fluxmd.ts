 ...
 *
 * 1. Managed memory (eg. /etc/flux-ai/CLAUDE.md) - Global instructions for all users
 ...
 */
function isFluxMdExcluded(filePath: string, type: MemoryType): boolean {
...
function resolveExcludePatterns(patterns: string[]): string[] {
  ...
  for (const normalized of expanded) {
    ...
    try {
      // sync IO: called from sync context (isFluxMdExcluded -> processMemoryFile -> getMemoryFiles)
...
export async function processMemoryFile(
  ...
  if (isFluxMdExcluded(filePath, type)) {
    return []
  }
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // See: https://github.com/anthropics/flux-ai/issues/29599
    ...
    // This is controlled by FLUX_AI_ADDITIONAL_DIRECTORIES_CLAUDE_MD and defaults to off
    // Note: we don't check isSettingSourceEnabled('projectSettings') here because --add-dir
    // is an explicit user action and the SDK defaults settingSources to [] when not specified
    if (isEnvTruthy(process.env.FLUX_AI_ADDITIONAL_DIRECTORIES_CLAUDE_MD)) {
