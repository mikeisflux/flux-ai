...
import { getFluxAIUserAgent } from './userAgent.js'
...
export function getUserAgent(): string {
  ...
  return `claude-cli/${MACRO.VERSION} (${process.env.USER_TYPE}, ${process.env.FLUX_AI_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}

export function getMCPUserAgent(): string {
  const parts: string[] = []
  if (process.env.FLUX_AI_ENTRYPOINT) {
    parts.push(process.env.FLUX_AI_ENTRYPOINT)
  }
  ...
  return `flux-ai/${MACRO.VERSION}${suffix}`
}
...
// operators match in robots.txt); the flux-ai suffix lets them distinguish
// local CLI traffic from claude.ai server-side fetches.
export function getWebFetchUserAgent(): string {
  return `Claude-User (${getFluxAIUserAgent()}; +https://support.anthropic.com/)`
}
