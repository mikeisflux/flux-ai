 ...
 * FLUX_AI_PROVIDER_MANAGED_BY_HOST in spawn env), strip
...
function withoutHostManagedProviderVars(
  ...
  if (!env) return {}
  if (!isEnvTruthy(process.env.FLUX_AI_PROVIDER_MANAGED_BY_HOST)) {
    return env
  }
 ...
 * traffic (e.g., FLUX_BASE_URL) to an attacker-controlled server.
 */
 ...
 * FLUX_BASE_URL take effect during first-run/onboarding.
 *
 * For trusted sources (user settings, managed settings, CLI flags), ALL env vars
 * are applied — including ones like FLUX_BASE_URL that would be dangerous
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // flagSettings env applied. Eligibility reads FLUX_AI_USE_BEDROCK,
  // FLUX_BASE_URL — both settable via settings.env.
  ...
  // when FLUX_AI_PROVIDER_MANAGED_BY_HOST is set.
 ...
 * FLUX_AI_PROVIDER_MANAGED_BY_HOST is set — see filterSettingsEnv) and
