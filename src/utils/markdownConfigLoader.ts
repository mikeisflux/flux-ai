 ...
 * See: https://github.com/anthropics/flux-ai/issues/13893
 *
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    // (anthropics/flux-ai#29599, #28182, #26992).
    //
 ...
 * 3. Can be explicitly enabled via FLUX_AI_USE_NATIVE_FILE_SEARCH env var
 *
 * Symlink handling:
...
> {
  ...
  // - Fallback: native Node.js (when FLUX_AI_USE_NATIVE_FILE_SEARCH is set)
  //
  // Why both? Ripgrep has poor startup performance in native builds.
  const useNative = isEnvTruthy(process.env.FLUX_AI_USE_NATIVE_FILE_SEARCH)
