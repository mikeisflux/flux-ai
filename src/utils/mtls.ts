...
export const getMTLSConfig = memoize((): MTLSConfig | undefined => {
  ...
  if (process.env.FLUX_AI_CLIENT_CERT) {
    try {
      ...
      logForDebugging(
        'mTLS: Loaded client certificate from FLUX_AI_CLIENT_CERT',
      )
    } catch (error) {
  ...
  // Client key
  if (process.env.FLUX_AI_CLIENT_KEY) {
    try {
      config.key = getFsImplementation().readFileSync(
        process.env.FLUX_AI_CLIENT_KEY,
        { encoding: 'utf8' },
      )
      logForDebugging('mTLS: Loaded client key from FLUX_AI_CLIENT_KEY')
    } catch (error) {
  ...
  // Key passphrase
  if (process.env.FLUX_AI_CLIENT_KEY_PASSPHRASE) {
    config.passphrase = process.env.FLUX_AI_CLIENT_KEY_PASSPHRASE
