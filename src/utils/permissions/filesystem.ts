...
export function isFluxSettingsPath(filePath: string): boolean {
...
function isFluxConfigFilePath(filePath: string): boolean {
  if (isFluxSettingsPath(filePath)) {
    return true
  }
...
// and per-turn from BashTool prompt. Inputs (FLUX_AI_TMPDIR env + platform) are
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isFluxConfigFilePath(pathToCheck)) {
      return {
        safe: false,
