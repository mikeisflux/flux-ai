 ...
 * `<flux-ai-hint />` tag to stderr (detected by the Bash/PowerShell tools).
 *
 * State persists in GlobalConfig.fluxAIHints — a show-once record per
...
/**
 * Hard cap on `fluxAIHints.plugin[]` — bounds config growth. Each shown
...
export function markHintPluginShown(pluginId: string): void {
  saveGlobalConfig(current => {
    const existing = current.fluxAIHints?.plugin ?? []
...
export function disableHintRecommendations(): void {
  saveGlobalConfig(current => {
    if (current.fluxAIHints?.disabled) return current
    return {
      ...current,
      fluxAIHints: { ...current.fluxAIHints, disabled: true },
    }
  })
}
