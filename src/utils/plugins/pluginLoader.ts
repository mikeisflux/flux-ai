...
async function installFromGitHub(
  ...
  const gitUrl = isEnvTruthy(process.env.FLUX_AI_REMOTE)
 ...
 * FLUX_AI_REMOTE) or any URL that passes validateGitUrl (https, http,
 * file, git@ ssh).
 */
 ...
 *
 * FLUX_AI_SYNC_PLUGIN_INSTALL=1 delegates to the full loader — that
 * mode explicitly opts into blocking install before first query, and
 * main.tsx's getFluxAIMcpConfigs()/getInitialSettings().agent run
