 ...
 * When FLUX_AI_PLUGIN_USE_ZIP_CACHE is enabled and FLUX_AI_PLUGIN_CACHE_DIR
...
export function isPluginZipCacheEnabled(): boolean {
  return isEnvTruthy(process.env.FLUX_AI_PLUGIN_USE_ZIP_CACHE)
}

/**
 * Get the path to the zip cache directory.
 * Requires FLUX_AI_PLUGIN_CACHE_DIR to be set.
...
export function getPluginZipCachePath(): string | undefined {
  ...
  const dir = process.env.FLUX_AI_PLUGIN_CACHE_DIR
...
async function collectFilesForZip(
  ...
  // See also: markdownConfigLoader.ts getFileIdentity, anthropics/flux-ai#13893
  try {
