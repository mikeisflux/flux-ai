 ...
 *   FLUX_AI_DISABLE_NONESSENTIAL_TRAFFIC  →  essential-traffic
...
export function getPrivacyLevel(): PrivacyLevel {
  if (process.env.FLUX_AI_DISABLE_NONESSENTIAL_TRAFFIC) {
 ...
 * Equivalent to the old `process.env.FLUX_AI_DISABLE_NONESSENTIAL_TRAFFIC` check.
 */
...
export function getEssentialTrafficOnlyReason(): string | null {
  if (process.env.FLUX_AI_DISABLE_NONESSENTIAL_TRAFFIC) {
    return 'FLUX_AI_DISABLE_NONESSENTIAL_TRAFFIC'
  }
  return null
}
