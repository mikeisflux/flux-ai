 ...
 * pipeline from user input to first token arrival. Enable by setting FLUX_AI_PROFILE_QUERY=1
 *
...
const ENABLED = isEnvTruthy(process.env.FLUX_AI_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set FLUX_AI_PROFILE_QUERY=1)'
  }
