...
function getEntrypoint(): string | undefined {
  return process.env.FLUX_AI_ENTRYPOINT
}
...
class Project {
   ...
   * FLUX_AI_SKIP_PROMPT_HISTORY should suppress all transcript writes.
  ...
  private shouldSkipPersistence(): boolean {
    ...
    return (
      ...
      isSessionPersistenceDisabled() ||
      isEnvTruthy(process.env.FLUX_AI_SKIP_PROMPT_HISTORY)
    )
  }
...
export async function loadTranscriptFile(
  ...
  try {
    ...
    if (!isEnvTruthy(process.env.FLUX_AI_DISABLE_PRECOMPACT_SKIP)) {
    ...
    // FLUX_AI_DISABLE_PRECOMPACT_SKIP is set (that kill switch means
    ...
    if (
      !opts?.keepAllLeaves &&
      !hasPreservedSegment &&
      !isEnvTruthy(process.env.FLUX_AI_DISABLE_PRECOMPACT_SKIP) &&
...
export function isLoggableMessage(m: Message): boolean {
  ...
  if (m.type === 'attachment' && getUserType() !== 'ant') {
    if (
      ...
      isEnvTruthy(process.env.FLUX_AI_SAVE_HOOK_ADDITIONAL_CONTEXT)
    ) {
      return true
    }
    return false
