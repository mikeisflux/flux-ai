...
/**
 * Convert a PersistedStatsCache to FluxAIStats by computing derived fields.
 */
 ...
 */
export async function aggregateFluxAIStats(): Promise<FluxAIStats> {
 ...
 */
export async function aggregateFluxAIStatsForRange(
  range: StatsDateRange,
): Promise<FluxAIStats> {
  if (range === 'all') {
    return aggregateFluxAIStats()
  }
  ...
  return processedStatsToFluxAIStats(stats)
}

/**
 * Convert ProcessedStats to FluxAIStats.
 * Used for filtered date ranges that bypass the cache.
 */
function processedStatsToFluxAIStats(
...
function getEmptyStats(): FluxAIStats {
  return {
    totalSessions: 0,
    totalMessages: 0,
    totalDays: 0,
