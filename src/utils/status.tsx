...
import { getAccountInformation, isFluxAISubscriber } from './auth.js';
import { getLargeMemoryFiles, getMemoryFiles, MAX_MEMORY_CHARACTER_COUNT } from './fluxmd.js';
...
export function buildAPIProviderProperties(): Property[] {
  ...
  if (apiProvider === 'firstParty') {
    const anthropicBaseUrl = process.env.FLUX_BASE_URL;
    if (anthropicBaseUrl) {
      properties.push({
    ...
    if (isEnvTruthy(process.env.FLUX_AI_SKIP_BEDROCK_AUTH)) {
      properties.push({
    ...
    const gcpProject = process.env.FLUX_VERTEX_PROJECT_ID;
    if (gcpProject) {
      properties.push({
        label: 'GCP project',
        value: gcpProject
  ...
  if (mtlsConfig) {
    if (mtlsConfig.cert && process.env.FLUX_AI_CLIENT_CERT) {
      properties.push({
        label: 'mTLS client cert',
        value: process.env.FLUX_AI_CLIENT_CERT
      });
    }
    if (mtlsConfig.key && process.env.FLUX_AI_CLIENT_KEY) {
      properties.push({
        label: 'mTLS client key',
        value: process.env.FLUX_AI_CLIENT_KEY
      });
    }
  }
  return properties;
...
export function getModelDisplayLabel(mainLoopModel: string | null): string {
  ...
  if (mainLoopModel === null && isFluxAISubscriber()) {
