 ...
 * via shell expansion (e.g., ${FLUX_API_KEY}) in Bash tool commands.
 *
...
const GHA_SUBPROCESS_SCRUB = [
  ...
  // flux-ai-action-specific duplicates — action JS consumes these during
 ...
 * servers, shell hooks).
 *
 * Gated on FLUX_AI_SUBPROCESS_ENV_SCRUB. flux-ai-action sets this
...
export function subprocessEnv(): NodeJS.ProcessEnv {
  ...
  if (!isEnvTruthy(process.env.FLUX_AI_SUBPROCESS_ENV_SCRUB)) {
  ...
  for (const k of GHA_SUBPROCESS_SCRUB) {
    ...
    // secrets like INPUT_FLUX_API_KEY. No-op for vars that aren't action inputs.
    delete env[`INPUT_${k}`]
  }
  return env
}
