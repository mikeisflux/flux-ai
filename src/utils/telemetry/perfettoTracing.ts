 ...
 *
 * Usage:
 * 1. Enable via FLUX_AI_PERFETTO_TRACE=1 or FLUX_AI_PERFETTO_TRACE=<path>
 * 2. Optionally set FLUX_AI_PERFETTO_WRITE_INTERVAL_S=<positive integer> to write the
...
export function initializePerfettoTracing(): void {
  const envValue = process.env.FLUX_AI_PERFETTO_TRACE
  logForDebugging(
  ...
  if (feature('PERFETTO_TRACING')) {
    ...
    // Start periodic full-trace write if FLUX_AI_PERFETTO_WRITE_INTERVAL_S is a positive integer
    const intervalSec = parseInt(
      process.env.FLUX_AI_PERFETTO_WRITE_INTERVAL_S ?? '',
      10,
    )
    if (intervalSec > 0) {
