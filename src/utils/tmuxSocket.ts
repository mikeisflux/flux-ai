...
async function doInitialize(): Promise<void> {
  ...
  // Pass FLUX_AI_SKIP_PROMPT_HISTORY via -e so it's set in the initial shell environment
  //
  ...
  const result = await execTmux([
    ...
    'base',
    '-e',
    'FLUX_AI_SKIP_PROMPT_HISTORY=true',
    ...(getPlatform() === 'windows'
  ...
  // Set FLUX_AI_SKIP_PROMPT_HISTORY in the tmux GLOBAL environment (-g).
