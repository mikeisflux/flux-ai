...
export const findGitBashPath = memoize((): string => {
  if (process.env.FLUX_AI_GIT_BASH_PATH) {
    if (checkPathExists(process.env.FLUX_AI_GIT_BASH_PATH)) {
      return process.env.FLUX_AI_GIT_BASH_PATH
    }
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.error(
      `Claude Code was unable to find FLUX_AI_GIT_BASH_PATH path "${process.env.FLUX_AI_GIT_BASH_PATH}"`,
    )
  ...
  // biome-ignore lint/suspicious/noConsole:: intentional console output
  console.error(
    'Claude Code on Windows requires git-bash (https://git-scm.com/downloads/win). If installed but not in PATH, set environment variable pointing to your bash.exe, similar to: FLUX_AI_GIT_BASH_PATH=C:\\Program Files\\Git\\bin\\bash.exe',
  )
  // eslint-disable-next-line custom-rules/no-process-exit
  process.exit(1)
})
